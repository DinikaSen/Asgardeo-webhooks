...
function processEventByType(string eventType, json eventData, SecurityEventToken setPayload) returns error? {
    ...
    if eventType.includes("registrationSuccess") {
        check processRegistrationSuccessEvent(eventData, setPayload);
...
function processRegistrationSuccessEvent(json eventData, SecurityEventToken setPayload) returns error? {
    ...
    if regEvent is error {
        return error("Failed to parse registration success event data");
    }

    log:printInfo("Registration success event processed",
